...
namespace xsd
{
  namespace cxx
  {
    namespace tree
    {
      ...
      template <typename C>
      inline void
      operator<< (xsd::cxx::tree::list_stream<C>& ls, long double d)
      {
        // xsd:decimal (which we currently map to long double)
        // can not be in scientific notation.
        //
      ...
      template <typename B>
      inline void
      operator<< (xercesc::DOMElement&, const simple_type<B>&)
      {
      }
      ...
      namespace bits
      {
        template <typename C>
        std::basic_string<C>
        resolve_prefix (const std::basic_string<C>& ns,
